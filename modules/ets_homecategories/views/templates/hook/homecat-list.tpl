...
<div class="block products_block ets_home_categories clearfix homecat-tab-list {if $ETS_HOMECAT_LOADING_ENABLED}loading-enabled{else}loading-disabled{/if}">	
	{if $ETS_HOMECAT_ENBLE_ALL_PRODUCT_TAB || $ETS_HOMECAT_DISPLAY_SUB && $categories}
        ...
        {foreach from=$categories item='category'}
            <li class="is_parent_cat_li is_parent_cat_li_{$category.id_category|intval}" data-id-category="{$category.id_category|intval}"><a class="{if !$ETS_HOMECAT_OPEN_CAT_BY_LINK}homecat_ajax_tab_list homecat_ajax_tab_list_{$category.id_category|intval}{/if} is_parent_cat homecat_tab_name homcat_tab_{$category.id_category|intval}" href="{$link->getCategoryLink($category.id_category)|escape:'html':'UTF-8'}" data-id-category="{$category.id_category|intval}">{$category.name|escape:'html'}</a>
                {if $ETS_HOMECAT_DISPLAY_SUB}
