...
class Ybc_widget extends Module
{
	...
	{	   
        ...
        $this->_hooks = array(
            ...
            array(
                'hook' => 'ybcCustom1',
                'name' => $this->l('Services on footer')
            ),
            array(
                'hook' => 'ybcCustom2',
                'name' => $this->l('Most searched keywords')
            ), 
            array(
                'hook' => 'ybcCustom3',
    ...
    public function getFieldsValues($formFields, $primaryKey, $objClass, $saveBtnName)
	{
        ...
        foreach($formFields as $field)
        {
            if(!isset($field['primary_key']) && !isset($field['multi_lang']) && !isset($field['connection']))
            {
                ...
                $fields[$field['name']] = Tools::getValue($field['name'], $obj->$fieldName) && is_string(Tools::getValue($field['name'], $obj->$fieldName)) ? trim(Tools::getValue($field['name'], $obj->$fieldName)) : Tools::getValue($field['name'], $obj->$fieldName);
            }
