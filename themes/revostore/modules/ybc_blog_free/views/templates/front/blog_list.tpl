                  ...
                  {block name="content"}
                    <div class="ybc_blog_free_layout_{$blog_layout|escape:'html':'UTF-8'} ybc-blog-wrapper ybc-blog-wrapper-blog-list {if $blog_latest}ybc-page-latest{elseif $blog_category}ybc-page-category{elseif $blog_tag}ybc-page-tag{elseif $blog_search}ybc-page-search{elseif $author}ybc-page-author{else}ybc-page-home{/if}">
                        ...
                        {if $blog_category}
                            ...
                            {else}
                                <p class="alert alert-warning">{l s='This category is not available' d='Shop.Theme.Actions'}</p>
                            {/if}
                        {elseif $blog_latest}
                           <h1 class="page-heading product-listing">{l s='Latest posts' d='Shop.Theme.Actions'}</h1>
                        {elseif $blog_tag}
                            <h1 class="page-heading product-listing">{l s='Tag: ' d='Shop.Theme.Actions'}"{ucfirst($blog_tag)|escape:'html':'UTF-8'}"</h1>
                        {elseif $blog_search}
                            <h1 class="page-heading product-listing">{l s='Search: ' d='Shop.Theme.Actions'}"{ucfirst($blog_search)|escape:'html':'UTF-8'}"</h1>
                        {elseif $author}
                            <h1 class="page-heading product-listing">{l s='Author: ' d='Shop.Theme.Actions'}"{$author|escape:'html':'UTF-8'}"</h1>
                        {/if}
                        ...
                        {if !($blog_category && (!isset($blog_category.enabled) || isset($blog_category.enabled) && !$blog_category.enabled)) && ($blog_category || $blog_tag || $blog_search || $author || $is_main_page || $blog_latest)}
                            {if isset($blog_posts) && $blog_posts}
                                            ...
                                            <div class="post-wrapper">
                                                ...
                                                <div class="ybc-blog-wrapper-content-main">
                                                    ...
                                                    <div class="ybc-blog-latest-toolbar">	
                    									{if $show_views}                    
                                                                <span class="ybc-blog-latest-toolbar-views" title="{l s='Page views' d='Shop.Theme.Actions'}">
                                                                    {$post.click_number|intval}
                                                                    {if $post.click_number !=1}<span>
                                                                        {l s='Views' d='Shop.Theme.Actions'}</span>
                                                                    {else}
                                                        ...
                                                        {if $allow_rating}
                                                            {if $post.total_review}
                                                                <span title="{l s='Comments' d='Shop.Theme.Actions'}" class="blog__rating_reviews">
                                                                     {$post.total_review|intval}
                                                                </span>
                                                            {/if}
                                                        {/if}
                                                        {if $allow_like}
                                                            <span title="{if $post.liked}{l s='Liked' d='Shop.Theme.Actions'}{else}{l s='Like this post' d='Shop.Theme.Actions'}{/if}" class="item ybc-blog-like-span ybc-blog-like-span-{$post.id_post|escape:'html':'UTF-8'} {if $post.liked}active{/if}"  data-id-post="{$post.id_post|escape:'html':'UTF-8'}">                        
                                                                <span class="blog-post-total-like ben_{$post.id_post|escape:'html':'UTF-8'}">{$post.likes|escape:'html':'UTF-8'}</span>
                                                                <span class="blog-post-like-text blog-post-like-text-{$post.id_post|escape:'html':'UTF-8'}"><span>{l s='Liked' d='Shop.Theme.Actions'}</span></span>
                                                            </span> 
                                                        {/if}                     
                                                        {if $allow_rating && isset($post.everage_rating) && $post.everage_rating}
                                                            ...
                                                            <div class="blog-extra-item be-rating-block item">
                                                                ...
                                                                <div class="blog_rating_wrapper">
                                                                    <div class="ybc_blog_free_review" title="{l s='Everage rating' d='Shop.Theme.Actions'}">
                                ...
                                {/if}
                            {else}
                                <p>{l s='No posts found' d='Shop.Theme.Actions'}</p>
                            {/if}
                        {/if}
                    </div>                
                  {/block}
