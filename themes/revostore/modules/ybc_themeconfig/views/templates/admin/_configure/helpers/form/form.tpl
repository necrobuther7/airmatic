...
{block name="field"}   
    ...
    {elseif $input.name=='IMPORT_DATA'}
        <div class="col-lg-9">
            {if isset($input.sections) && $input.sections}
                ...
                {/foreach}
                <input type="submit" name="SUBMIT_IMPORT" value="{l s='Import sample data' d='Shop.Theme.Actions'}" class="button btn btn-default" id="ybc_submit_import" />
                {if isset($devMode) && $devMode}
                    <input type="submit" name="SUBMIT_EXPORT" value="{l s='Export sample data to sql files' d='Shop.Theme.Actions'}" class="button btn btn-default" id="ybc_submit_export" />
                {/if}
                <div style="display: none;" id="ybc_import_warning_msg">{l s='You are going to overried your old data of the selected section(s). Do you confirm?'  d='Shop.Theme.Actions'}</div>
                <div style="display: none;" id="ybc_export_warning_msg">{l s='You are going to overried all sample data. Do you confirm?'  d='Shop.Theme.Actions'}</div>
    ...
    {elseif $input.type == 'file' &&  isset($input.display_img) && $input.display_img}
        {$smarty.block.parent}
        <label class="control-label col-lg-3" style="font-style: italic;">{l s='Uploaded image: ' d='Shop.Theme.Actions'}</label>
        <div class="col-lg-9">
    		<a  class="ybc_fancy" href="{$input.display_img|escape:'html':'UTF-8'}"><img title="{l s='Click to see full size image' d='Shop.Theme.Actions'}" style="display: inline-block; max-width: 200px;" src="{$input.display_img|escape:'html':'UTF-8'}" /></a>
            {if isset($input.img_del_link) && $input.img_del_link && !(isset($input.required) && $input.required)}
                <a onclick="return confirm('{l s='Do you want to delete this image?' d='Shop.Theme.Actions'}');" style="display: inline-block; text-decoration: none!important;" href="{$input.img_del_link|escape:'html':'UTF-8'}"><span style="color: #666"><i style="font-size: 20px;" class="process-icon-delete"></i></span></a>
            {/if}
        </div>
	...
	{if isset($fieldset['form']['submit']) || isset($fieldset['form']['buttons'])}
		<div class="panel-footer">
            {if isset($reset_url) && $reset_url}
                <a title="{l s='Reset to default parameters' d='Shop.Theme.Actions'}" onclick="return confirm('{l s='You are going to reset all parameters to default. Do you confirm?' d='Shop.Theme.Actions'}');" class="btn btn-default" href="{$reset_url|escape:'html':'UTF-8'}"><i class="process-icon-refresh"></i>{l s='Reset' d='Shop.Theme.Actions'}</a>
            {/if}
